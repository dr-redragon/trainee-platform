...
import { FileText, Video, LinkIcon, BookOpen, CheckSquare, FolderOpen, Plus } from "lucide-react";
        ...
        <Tabs defaultValue={defaultSubsections[0]} className="w-full">
          <TabsList className="w-full justify-start overflow-x-auto bg-secondary/50 p-1">
            {defaultSubsections.map((sub) => (
              <TabsTrigger key={sub} value={sub} className="text-xs whitespace-nowrap">
                {sub}
              </TabsTrigger>
            ))}
          </TabsList>
